...
# The base directory of the qxchain project
BASE_DIR="$SCRIPT_DIR/.."
...
"$BUILD_DIR/release/qxchain" build-spec --disable-default-bootnode --raw --chain "$CHAIN" >"$FULL_PATH"
echo "*** Chainspec built and output to file"

# Generate node keys
"$BUILD_DIR/release/qxchain" key generate-node-key --chain="$FULL_PATH" --base-path /tmp/alice
"$BUILD_DIR/release/qxchain" key generate-node-key --chain="$FULL_PATH" --base-path /tmp/bob

if [ $NO_PURGE -eq 1 ]; then
  echo "*** Purging previous state skipped..."
else
  echo "*** Purging previous state..."
  "$BUILD_DIR/release/qxchain" purge-chain -y --base-path /tmp/bob --chain="$FULL_PATH" >/dev/null 2>&1
  "$BUILD_DIR/release/qxchain" purge-chain -y --base-path /tmp/alice --chain="$FULL_PATH" >/dev/null 2>&1
