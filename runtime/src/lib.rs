...
use polkadot_sdk::frame_support::{
	...
	traits::{ConstBool, ConstU128, ConstU32, ConstU64, ConstU8, Get, VariantCountOf, Contains},
	pallet_prelude::One,
	weights::{
...
/// Calls that should be allowed to bypass fee payment (for onboarding new users)
pub struct FeelessCalls;
impl Contains<RuntimeCall> for FeelessCalls {
	fn contains(call: &RuntimeCall) -> bool {
		matches!(call, RuntimeCall::QxFaucet(pallet_qx_faucet::Call::claim { .. }))
	}
}

/// Configure skip-feeless-payment pallet
impl polkadot_sdk::pallet_skip_feeless_payment::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
}

/// The SignedExtension to the basic transaction logic.
/// Uses SkipCheckIfFeeless to allow faucet claims without needing existing balance.
pub type SignedExtra = (
	...
	polkadot_sdk::pallet_skip_feeless_payment::SkipCheckIfFeeless<
		Runtime,
...
parameter_types! {
	/// 10 QX per faucet claim (10 * 10^12 units)
	pub const FaucetDripAmount: Balance = 10_000_000_000_000;
	/// 24 hours between claims (at 6s blocks = 14400 blocks)
	pub const FaucetClaimCooldown: BlockNumber = 14400;
	/// Maximum 5 lifetime claims per account (50 QX total)
	pub const FaucetMaxClaims: u32 = 5;
}

/// Faucet pot account - derived from a fixed seed
/// This account must be pre-funded in genesis
pub struct FaucetPotAccount;
impl Get<AccountId> for FaucetPotAccount {
	fn get() -> AccountId {
		// Use a deterministic account derived from "qx/faucet" seed
		// In production, this should be a proper pre-generated account
...
/// Configure the QX Faucet pallet
impl pallet_qx_faucet::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type Currency = Balances;
	type DripAmount = FaucetDripAmount;
	type ClaimCooldown = FaucetClaimCooldown;
	type MaxClaimsPerAccount = FaucetMaxClaims;
	type FaucetPot = FaucetPotAccount;
}
...
mod runtime {
	...
	pub type QxFaucet = pallet_qx_faucet;

	#[runtime::pallet_index(10)]
	pub type SkipFeelessPayment = polkadot_sdk::pallet_skip_feeless_payment;
}
